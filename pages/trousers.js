...
const Trousers = ({ data }) => {
  return (
    <div className="min-h-screen">
      <Head>
        <title>Buy Cool Trousers - LoneStore.com</title>
        <meta
          name="viewport"
        ...
        <div className="container px-5 py-24 mx-auto">
          {Object.keys(data).length === 0 && (
            <p className="text-center font-bold">
              Trousers are currently out of stock!! New stock coming soon
            </p>
          )}
                  ...
                  <div className="p-4 my-6 w-full cursor-pointer shadow-2xl">
                    ...
                    <div className="mt-4">
                      ...
                      <div className="mt-1 flex">
                        ...
                        {data[item].color.includes("gray") && (
                          <button
                            className={
                              "border-2 border-gray-300 bg-gray-700 my-auto ml-1 rounded-full w-6 h-6 focus:outline-none"
                            }
                          ></button>
                        )}
                      </div>
...
export async function getServerSideProps() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_host}/api/getproducts/Trouser`
  );
